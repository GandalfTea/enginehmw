...
#ifdef VERBOSE
    cout << "\t\t" << b.at(0, 0) << " : " <<  "-0.730132884184922"   << std::endl; 
    cout << "\t\t" << b.at(0, 1) << " : " <<   "0.5755227451396153"  << std::endl; 
    cout << "\t\t" << b.at(0, 2) << " : " <<  "-0.3683470391611273"  << std::endl; 
    cout << "\t\t" << b.at(1, 0) << " : " <<  "-0.5778871754659348"  << std::endl; 
    cout << "\t\t" << b.at(1, 1) << " : " <<  "-0.8077470099558122"  << std::endl;
    cout << "\t\t" << b.at(1, 2) << " : " <<  "-0.11658121778163623" << std::endl;
    cout << "\t\t" << b.at(2, 0) << " : " <<  "-0.36462636199788356" << std::endl;
    cout << "\t\t" << b.at(2, 1) << " : " <<   "0.12774324927136735" << std::endl;
    cout << "\t\t" << b.at(2, 2) << " : " <<   "0.922349867675917"   << std::endl;  
#endif

  assert( std::fmod( b.at(0, 0), -0.730132884184922 )  <= 0.00005 );
  assert( std::fmod( b.at(0, 1),  0.5755227451396153)  <= 0.00005 );
  assert( std::fmod( b.at(0, 2), -0.3683470391611273)  <= 0.00005 );
  assert( std::fmod( b.at(1, 0), -0.5778871754659348)  <= 0.00005 );
  assert( std::fmod( b.at(1, 1), -0.8077470099558122)  <= 0.00005 );
  assert( std::fmod( b.at(1, 2), -0.11658121778163623) <= 0.00005 );
  assert( std::fmod( b.at(2, 0), -0.36462636199788356) <= 0.00005 );
  assert( std::fmod( b.at(2, 1),  0.12774324927136735) <= 0.00005 );
  assert( std::fmod( b.at(2, 2),  0.922349867675917)   <= 0.00005 );
...
bool mat_to_quat() {
  ...
  Quaternion b (a);
  assert( std::fmod( b.s,  0.19701792944740668) <= 0.00005 );
  assert( std::fmod( b.v1, -0.0030002730372701526) <= 0.00005 );
  assert( std::fmod( b.v2, -0.9300846415537474) <= 0.00005 );
  assert( std::fmod( b.v3,  0.31002821385124907) <= 0.00005 ); 
  return true;
}
